...
export default function Login() {
    ...
    async function handleSubmit(e) {
        ...
        if (isSignUp) {
            ...
            if (error) setError(error.message)
                ...
                setError('Account created. Please verify your email and then sign in.')
            }
        } else {
    ...
    async function handleGoogle() {
        const { error } = await supabase.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: `${window.location.origin}/${selectedRole === 'owner' ? 'dashboard' : selectedRole === 'super_admin' ? 'admin' : 'tenant'}`
            }
        })
        if (error) setError(error.message)
    }
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-500 text-sm">Choose a role to continue</p>
                </div>
