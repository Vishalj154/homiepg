...
export default function Profile() {
  ...
  const [form, setForm] = useState({
    full_name: '', phone: '', business_name: '', city: '', state: '',
  })
  ...
  async function handleSave(e) {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      business_name: form.business_name, city: form.city, state: form.state,
    }).eq('id', user.id)
    if (error) await supabase.from('profiles').update({ full_name: form.full_name }).eq('id', user.id)
    setSaved(true)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                {form.business_name && <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{form.business_name}</p>}
              </div>
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6">
                <h2 className="font-semibold text-gray-700 dark:text-gray-100 mb-4">Profile Details</h2>
                <form onSubmit={handleSave} className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    {saved && <span className="text-homie-green text-sm font-medium">✅ Saved!</span>}
                  </div>
                </form>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 border border-red-100 dark:border-red-700">
                ...
                <p className="text-sm text-gray-400 mb-4">You'll be returned to the HomiePG homepage.</p>
